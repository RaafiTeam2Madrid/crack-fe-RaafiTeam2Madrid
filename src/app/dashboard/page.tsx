...
export default function DashboardPage() {
  ...
  // 1. Menarik data dari gudang arsip dan akses (DENGAN TIKET SATPAM)
  useEffect(() => {
    // Ambil sesi user dan token JWT dari Cookie
    ...
    const token = getCookie("accessToken"); // 👈 Mengambil tiket (ID Card)
    ...
    const fetchData = async () => {
      try {
        // 👈 Menyiapkan urat nadi (headers) yang berisi tiket
        const headers = {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
        };

        // 👈 Kurir (fetch) sekarang membawa tiket ke Satpam Backend
        const [resArchives, resRequests] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/archives`, { headers }),
  ...
  // 2. Mesin Kalkulasi Statistik Admin
  const totalArsip = archives.length;
  const aktif = archives.filter(
    (a) => (a.category || "").toLowerCase() === "aktif",
  ).length;
  const inaktif = archives.filter(
    (a) => (a.category || "").toLowerCase() === "inaktif",
  ).length;
  const vital = archives.filter(
    (a) => (a.category || "").toLowerCase() === "vital",
  ).length;

  const pendingRequestsAdmin = accessRequests.filter(
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "PENDING":
        return (
          <span className="px-3 py-1 bg-[#f59e0b]/20 text-[#f59e0b] border border-[#f59e0b]/50 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-2 w-max">
      ...
      case "APPROVED":
        return (
          <span className="px-3 py-1 bg-[#0a8270]/20 text-[#0a8270] border border-[#0a8270]/50 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-2 w-max">
            <span className="w-1.5 h-1.5 rounded-full bg-[#0a8270]"></span>{" "}
            DISETUJUI
          </span>
        );
      case "REJECTED":
        return (
          <span className="px-3 py-1 bg-[#eb3434]/20 text-[#eb3434] border border-[#eb3434]/50 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-2 w-max">
            <span className="w-1.5 h-1.5 rounded-full bg-[#eb3434]"></span>{" "}
            DITOLAK
          </span>
        );
      case "USED":
        return (
          <span className="px-3 py-1 bg-slate-800 text-slate-400 border border-slate-600 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-2 w-max">
      ...
      <div className="space-y-8 animate-[fadeIn_0.5s_ease-out]">
        <div className="border-b border-white/5 pb-6">
          <h1 className="text-3xl font-serif text-white tracking-tight mb-2">
            Selamat Datang,{" "}
            <span className="italic text-[#f59e0b]">{userName}.</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#1a1a1a] p-8 rounded-[2rem] border border-white/5 shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-slate-400 text-sm mb-6">
              Cari dan ajukan permohonan akses untuk dokumen yang Anda butuhkan.
            </p>
            <Link
              href="/dashboard/archives"
              className="inline-block px-6 py-3 bg-[#2358d8] hover:bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-900/50 hover:-translate-y-1"
            >
              Buka Katalog Arsip
            </Link>
          </div>
          ...
          <div className="bg-[#1a1a1a] p-8 rounded-[2rem] border border-white/5 shadow-2xl relative overflow-hidden">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-slate-400 text-[10px] font-black tracking-widest uppercase mb-1">
                  Tiket Aktif Anda
                </h3>
              ...
              <div className="w-12 h-12 rounded-full bg-[#f59e0b]/20 flex items-center justify-center text-[#f59e0b] text-xl">
                🎟️
              </div>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-white/5">
                {accessRequests.length === 0 ? (
                  ...
                  accessRequests.map((req) => (
                    ...
                    >
                      <td className="p-6 font-semibold text-white text-sm">
                        {req.archiveTitle}
                      </td>
                      <td className="p-6 text-slate-400 text-xs italic">
                        "{req.reason}"
                      </td>
    ...
    <div className="text-slate-200 font-sans min-h-full animate-[fadeIn_0.5s_ease-out]">
      ...
      {pendingRequestsAdmin.length > 0 && (
        <div className="bg-[#f59e0b]/10 border border-[#f59e0b]/30 p-6 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between mb-8 animate-pulse">
          <div>
            <h3 className="text-[#f59e0b] font-bold mb-1 flex items-center gap-2">
              ⚠️ Perhatian: Ada {pendingRequestsAdmin.length} Permintaan Akses
              Baru!
            </h3>
            <p className="text-slate-400 text-sm">
              Staf menunggu persetujuan Anda untuk membuka dokumen.
            </p>
          </div>
          <Link
            href="/dashboard/admin/access"
            className="mt-4 md:mt-0 px-5 py-2.5 bg-[#f59e0b] text-black rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-yellow-400 transition-colors"
          >
            Tinjau Sekarang
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        <div className="bg-[#eb3434] rounded-[2rem] p-8 relative overflow-hidden shadow-[0_10px_30px_rgba(235,52,52,0.3)] hover:-translate-y-2 transition-transform duration-300">
          ...
          <p className="text-white/90 text-[10px] font-black uppercase tracking-[0.2em] mb-6">
            Total Arsip
          </p>
          <h2 className="text-7xl font-serif italic text-white leading-none">
            {totalArsip}
          </h2>
        </div>
        ...
        <div className="bg-[#ffe227] rounded-[2rem] p-8 relative overflow-hidden shadow-[0_10px_30px_rgba(255,226,39,0.2)] hover:-translate-y-2 transition-transform duration-300">
          <p className="text-black/60 text-[10px] font-black uppercase tracking-[0.2em] mb-6">
            Arsip Vital
          </p>
          <h2 className="text-7xl font-serif italic text-black leading-none">
            {vital}
          </h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-[#1a1a1a] rounded-[2.5rem] p-8 border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <p className="text-white/80 text-[10px] font-black uppercase tracking-[0.2em] mb-8">
            Distribusi Klasifikasi Arsip
          </p>
        ...
        <div className="bg-[#1a1a1a] rounded-[2.5rem] p-8 border border-white/5 shadow-2xl flex flex-col">
            ...
            {recentArchives.length === 0 ? (
              ...
              recentArchives.map((archive, index) => {
                ...
                return (
                  <div
                    key={archive.id}
                    className="flex gap-4 items-center border-b border-white/10 pb-5 last:border-0 last:pb-0"
                  >
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xs shadow-lg ${colorClass}`}
                    >
                      {baseCode}
                    </div>
                    <div className="flex-1 overflow-hidden">
                      ...
                      <p className="text-slate-500 text-[10px] font-mono mt-1 uppercase">
                        {new Date(archive.uploadDate).toLocaleDateString(
                          "id-ID",
          ...
          <Link
            href="/dashboard/archives"
            className="w-full mt-6 py-4 rounded-xl border border-white/10 text-slate-400 hover:text-white hover:bg-white/5 text-[10px] font-black uppercase tracking-[0.2em] transition-all text-center block"
          >
            Lihat Daftar Arsip
          </Link>
        </div>
